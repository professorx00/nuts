...
This system is a nuts system that you can use as a starting point for building your own custom systems. It's similar to Simple World-building, but has examples of creating attributes in code rather than dynamically through the UI.

## Usage

There are two ways to get started: using the nuts system generator command or manually renaming and updating files.
...
> **Data Models**
>
> If you would like to use DataModel classes instead of the older template.json configuration, you'll need to use the `npm run generate` command described below and choose to enable them when asked. DataModels are currently an optional shell, and are only availabe in the generator CLI due to that.
...
> **Python Generator**
> 
> If you would rather use Python than node, there’s an excellent Python-based generator created by Cussa at https://github.com/Cussa/fvtt-nuts-initializator. Give it a shot!
...
Before installing this system, you should rename any files that have `nuts` in their filename to use whatever machine-safe name your system needs, such as `adnd2e` if you were building a system for 2nd edition Advanced Dungeons & Dragons. In addition, you should search through the files for `nuts` and `nuts` and do the same for those, replacing them with appropriate names for your system.
...
Head over to the [Vue3nuts System](https://gitlab.com/asacolips-projects/foundry-mods/vue3nuts) repo if you're interested in using Vue!
...
![image](http://mattsmith.in/images/nuts.png)
